...
export function OyunEkrani() {
  ...
  const spriteKey =
    g.sonuc || sahne.kind === 'yol' || sahne.kind === 'tanitim' || sahne.kind === 'ders'
      ? undefined
      : sahne.sprite;
    ...
    <>
      ...
      >
        {g.miniAktif && sahne.kind === 'mini' ? (
          ...
          <Pressable style={{ flex: 1 }} onPress={() => setAtla(true)} accessibilityRole="none">
            <ScrollView
              contentContainerStyle={{ padding: SP.lg, gap: SP.lg, flexGrow: 1 }}
              keyboardShouldPersistTaps="handled"
            >
              {/* Nerede olduğunu gösteren mekan kesiti; yoksa sahnenin kendi görseli */}
              {sahne.kind !== 'yol' && sahne.kind !== 'tanitim' && mekanVar ? (
                <MekanSeridi blokId={blok.id} saat={blok.from} />
              ) : (
                spriteKey && (
                  <View style={{ alignItems: 'center', paddingVertical: SP.md }}>
                    <PixelSprite
                      sprite={sprite(spriteKey)}
                      scale={spriteKey === 'kisla' || spriteKey === 'tufek' ? 7 : 6}
                    />
                  </View>
                )
              )}
              ...
              {g.sonuc ? (
                ...
                <PixelPanel style={{ padding: SP.lg }}>
                  {sahne.speaker && (
                    <PixelText font="command" size="lead" color={C.brass} style={{ marginBottom: SP.sm }}>
                ...
                <PixelPanel style={{ padding: SP.lg }}>
                  <PixelText font="command" size="h3" color={C.brass} style={{ marginBottom: SP.sm }}>
              ...
              {!g.sonuc && metinBitti && sahne.kind === 'serbest' && <SerbestSahnesi />}
              {!g.sonuc && metinBitti && sahne.kind === 'tanitim' && (
                <TanitimSahnesi onBitti={g.ileri} />
              )}
              {!g.sonuc && metinBitti && sahne.kind === 'ders' && <DersSahnesi onBitti={g.ileri} />}
              ...
              <View style={{ gap: SP.sm }}>
                {g.sonuc ? (
                  ...
                  <PixelButton label="Başla" onPress={g.miniBaslat} disabled={!metinBitti} />
                ) : sahne.kind === 'anlati' && sahne.choices?.length ? (
                  metinBitti &&
                  sahne.choices
                    .filter(
                      (c) =>
                        (!c.gerekliEsya || (g.envanter[c.gerekliEsya]?.adet ?? 0) > 0) &&
                        (!c.sadeceSigaraIcen || g.profil.sigaraIciyor),
                    )
                    .map((c) => (
                      <PixelButton key={c.id} tur="secim" label={c.label} onPress={() => g.secimYap(c)} />
                    ))
                ) : sahne.kind === 'anlati' ? (
                  <PixelButton label="Devam" onPress={g.ileri} disabled={!metinBitti} />
                ) : null}
              </View>
            </ScrollView>
          </Pressable>
  ...
  return (
    <Animated.View
      style={{
        ...
        gap: SP.sm,
        opacity: giris,
        transform: [{ translateX: giris.interpolate({ inputRange: [0, 1], outputRange: [-14, 0] }) }],
      }}
    >
